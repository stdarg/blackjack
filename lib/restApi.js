...
function logout(req, res, json) {
    ...
    try {
        ...
        if (info.player.tableId !== -1) {
            var hasId = Tables.all.hasOwnProperty(json.tableId);
            if (Tables.all.length <= json.tableId || !hasId) {
                res.json({success: false, cmd: 'logout',
                         error: 'No such table id: '+json.tableId});
                return false;
            }
