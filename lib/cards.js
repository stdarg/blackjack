...
Shoe.prototype.deal = function(num) {
    ...
    //debug('deal: self.cards.length: '+self.cards.length+', num: '+num);
 ...
 * @return {Number|Number[]} The value or values for the hand
 */
function evalHand(hand) {
    assert.ok(is.nonEmptyArray(hand));
    var numAces = _.reduce(hand, function(sum, card) {
    ...
    var valNoAces = _.reduce(hand, function(sum, card) {
        if (!is.obj(card) || is.undef(card.rank))
            return sum+0;
        assert.ok(is.nonEmptyStr(card.rank));
        if (card.rank.toLowerCase() === 'ace')
            return sum+0;
    ...
    //logger.debug('evalHand: valNoAces: '+valNoAces);

    if (numAces === 0) {
        logger.debug('evalHand: return: '+valNoAces);
        return valNoAces;
    }
    ...
    for (var i=0; i<numAces; i++) {
        ...
        for (var j=0; j<data.length; j++) {
            //logger.debug('adding data[j]+11 ='+(data[j]+11));
            newVals.push(data[j]+11);
            //logger.debug('adding data[j]+1 ='+(data[j]+1));
            newVals.push(data[j]+1);
        }
        //logger.debug('>>> newVals: '+inspect(newVals));
        //logger.debug('>>> before data: '+inspect(data));
        data = newVals.splice(0);
        //logger.debug('>>> after data: '+inspect(data));
    }
    ...
    logger.debug('evalHand: return: '+inspect(data));
    return data;
}
...
function isBusted(hand) {
    ...
    assert.ok(is.array(val));
    var num21OrLess = _.reduce(val, function(sum, v) {
        if (v > 21)
            return sum;
        return sum+1;
    }, 0);
    logger.debug('isBusted num21OrLess: %d', num21OrLess);
    if (num21OrLess > 0)
        return false;
    return true;
}
...
function belowHard17(hand) {
    ...
    if (is.num(val)) {
        logger.debug('belowHard17 result: '+(val < 17 ? true : false));
        return val < 17 ? true : false;
    }
    assert.ok(is.array(val));
    var min = 22;
    var max = 0;
    _.reduce(val, function(v) {
        min = (v < min) ? v : min;
        max = (v > max) ? v : max;
    }, 0);
    logger.debug('belowHard17 min result: '+(min < 17 ? true : false)+' - '+
                 min);
    logger.debug('belowHard17 max result: '+(max < 17 ? true : false)+' - '+
                 max);
    return (min < 17 ? true : false);
    //return max < 17 ? true : false;
}
...
function scoreHand(hand) {
    ...
    _.each(val, function(v) {
        max = (v <= 21 && v > max) ? v : max;
    });
    logger.debug('scoreHand: '+inspect(val));
    if (max === 0) {
        max = val[0];
    }
    return max;
