...
var debug = require('debug')('bj:cards');
...
function evalHand(hand) {
    ...
    //debug('evalHand: hand: '+inspect(hand));
    //debug('evalHand: numAces: '+numAces);
    ...
    //debug('evalHand: valNoAces: '+valNoAces);
    if (numAces === 0) {
        //debug('evalHand: return: '+valNoAces);
        return valNoAces;
    }
    var data = [];
    for (var i=0; i<numAces; i++) {
        if (valNoAces+11 <= 21)
            data.push(valNoAces+11);
        data.push(valNoAces+1);
        if (data.length === 1)
            data = data[0];
    }
    //debug('evalHand: return: '+inspect(data));
    return data;
}
...
function isBusted(hand) {
    ...
    var num21OrLess = _.countBy(val, function(v) {
        if (v > 21)
            return 0;
        return 1;
    });
...
function belowHard17(hand) {
    assert.ok(is.nonEmptyArray(hand));
    debug('belowHard17 hand: '+inspect(hand));
    var val = evalHand(hand);
    debug('belowHard17 hand val: '+inspect(val));
    if (is.num(val)) {
        debug('belowHard17 result: '+(val < 17 ? true : false));
        return val < 17 ? true : false;
    }
    assert.ok(is.array(val));
    var min = 22;
    _.reduce(val, function(v) {
        min = (v <= 21 && v < min) ? v : min;
    }, 0);
    debug('belowHard17 result: '+(min < 17 ? true : false));
