...
describe('shoe', function() {
    it('should construct a randomized deck of 52 cards', function(done) {
        ...
        for (var i=0; i<DealerShoe.cards.length; i++) {
            assert.ok(is.obj(DealerShoe.cards[i]));
            if (DealerShoe.cards[i].suit === cards.orderedDeck[i].suit &&
                DealerShoe.cards[i].rank === cards.orderedDeck[i].rank) {
                inOrderCnt++;
            }
        }

        var percentInOrder = Math.floor((inOrderCnt/DealerShoe.cards.length) * 100);
        if (percentInOrder > 5) {
    ...
    it('should construct 8 randomized decks of 52 cards each', function(done) {
        ...
        for (var i=0; i<DealerShoe.cards.length; i++) {
            assert.ok(is.obj(DealerShoe.cards[i]));
            var ocIdx = i%52;
            if (DealerShoe.cards[i].suit === cards.orderedDeck[ocIdx].suit &&
                DealerShoe.cards[i].rank === cards.orderedDeck[ocIdx].rank) {
                inOrderCnt++;
            }
        }
    ...
    it('should deal 1 or more cards, removing them from the top', function(done) {
        var DealerShoe = new cards.Shoe(1);
